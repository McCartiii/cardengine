...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/v1/admin/stats`, { headers: { Authorization: `Bearer ${(user as unknown as { access_token?: string }).access_token ?? ""}` } })
  ...
  async function triggerIngest() {
    ...
    try {
      const res = await fetch(`${API_BASE}/v1/admin/ingest`, { method: "POST", headers: { Authorization: `Bearer ${(user as unknown as { access_token?: string }).access_token ?? ""}` } });
      const data = await res.json();
      setIngestLog(data.message ?? JSON.stringify(data));
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="glass rounded-2xl p-6" style={{ border: "1px solid rgba(255,0,128,0.1)" }}>
        ...
        <button onClick={triggerIngest} disabled={ingestRunning} className="px-6 py-3 rounded-xl text-sm font-bold transition-all"
        ...
        {ingestLog && <p className="text-xs mt-4 font-mono" style={{ color: ingestRunning ? "#00d4ff" : "#50c878" }}>{ingestLog}</p>}
      </div>
    </div>
  );
}
